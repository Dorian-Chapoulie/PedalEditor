...
class Toto1Node extends AudioWorkletNode {

    constructor(context, baseURL, options)
    {
        super(context, 'Toto1', options);
    ...
    ctrlChange(channel, ctrl, value)
    {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, Toto1Node.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
...
class Toto1 {
    ...
    async load()
    {
        try {
            ...
            let real_url = (this.baseURL === "") ? "Toto1.wasm" : (this.baseURL + "/Toto1.wasm");
            ...
            return new Promise((resolve, reject) => {   
                ...
                let re = /JSON_STR/g;
                let Toto1ProcessorString1 = Toto1ProcessorString.replace(re, json);
                let real_url = window.URL.createObjectURL(new Blob([Toto1ProcessorString1], { type: 'text/javascript' }));
                
                this.context.audioWorklet.addModule(real_url).then(() => {
                    this.node = new Toto1Node(this.context, this.baseURL, 
                    ...
                    this.node.onprocessorerror = () => { console.log('An error from Toto1-processor was detected.');}
                    return (this.node);
                }).then((node) => {
                    resolve(node);
                }).catch((e) => {
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createToto1GUI(this.node);
                        resolve(element);
                    }
                } else {
...
let Toto1ProcessorString = `

    'use strict';

    // Monophonic Faust DSP
    class Toto1Processor extends AudioWorkletProcessor {
        
        // JSON parsing functions
        static parse_ui(ui, obj, callback)
        {
            for (var i = 0; i < ui.length; i++) {
                Toto1Processor.parse_group(ui[i], obj, callback);
            }
        }
        ...
        {
            if (group.items) {
                Toto1Processor.parse_items(group.items, obj, callback);
            }
        }
        ...
        static parse_item1(item, obj, callback)
        {
            if (item.type === "vgroup"
                || item.type === "hgroup"
                || item.type === "tgroup") {
                Toto1Processor.parse_items(item.items, obj, callback);
            } else if (item.type === "hbargraph"
                       || item.type === "vbargraph") {
                // Nothing
            } else if (item.type === "vslider"
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            Toto1Processor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, Toto1Processor.parse_item1);
            return params;
        }
        ...
        constructor(options)
        {
            ...
            this.Toto1_instance = new WebAssembly.Instance(options.processorOptions.wasm_module, importObject);
            ...
            this.factory = this.Toto1_instance.exports;
            this.HEAP = this.Toto1_instance.exports.memory.buffer;
            ...
            this.initAux = function ()
            {
                ...
                // Parse UI
                Toto1Processor.parse_ui(this.json_object.ui, this, Toto1Processor.parse_item2);
    ...
    registerProcessor('Toto1', Toto1Processor);
`;

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.Toto1 = Toto1;
    window.FaustToto1 = Toto1;
} else {
    module.exports = { Toto1 };
}
