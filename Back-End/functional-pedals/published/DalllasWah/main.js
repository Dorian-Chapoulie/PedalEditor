...
class DalllasWahNode extends AudioWorkletNode {

    constructor(context, baseURL, options)
    {
        super(context, 'DalllasWah', options);
    ...
    ctrlChange(channel, ctrl, value)
    {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, DalllasWahNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel)
    {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, DalllasWahNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class DalllasWah {
    ...
    async load()
    {
        try {
            ...
            let real_url = (this.baseURL === "") ? "DalllasWah.wasm" : (this.baseURL + "/DalllasWah.wasm");
            ...
            return new Promise((resolve, reject) => {   
                ...
                let re = /JSON_STR/g;
                let DalllasWahProcessorString1 = DalllasWahProcessorString.replace(re, json);
                let real_url = window.URL.createObjectURL(new Blob([DalllasWahProcessorString1], { type: 'text/javascript' }));
                
                this.context.audioWorklet.addModule(real_url).then(() => {
                    this.node = new DalllasWahNode(this.context, this.baseURL, 
                    ...
                    this.node.onprocessorerror = () => { console.log('An error from DalllasWah-processor was detected.');}
                    return (this.node);
                }).then((node) => {
                    resolve(node);
                }).catch((e) => {
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createDalllasWahGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let DalllasWahProcessorString = `

    'use strict';

    // Monophonic Faust DSP
    class DalllasWahProcessor extends AudioWorkletProcessor {
        
        // JSON parsing functions
        static parse_ui(ui, obj, callback)
        {
            for (var i = 0; i < ui.length; i++) {
                DalllasWahProcessor.parse_group(ui[i], obj, callback);
            }
        }
        ...
        {
            if (group.items) {
                DalllasWahProcessor.parse_items(group.items, obj, callback);
            }
        }
        ...
        static parse_item1(item, obj, callback)
        {
            if (item.type === "vgroup"
                || item.type === "hgroup"
                || item.type === "tgroup") {
                DalllasWahProcessor.parse_items(item.items, obj, callback);
            } else if (item.type === "hbargraph"
                       || item.type === "vbargraph") {
                // Nothing
            } else if (item.type === "vslider"
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            DalllasWahProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, DalllasWahProcessor.parse_item1);
            return params;
        }
        ...
        constructor(options)
        {
            ...
            this.DalllasWah_instance = new WebAssembly.Instance(options.processorOptions.wasm_module, importObject);
            ...
            this.factory = this.DalllasWah_instance.exports;
            this.HEAP = this.DalllasWah_instance.exports.memory.buffer;
            ...
            this.initAux = function ()
            {
                ...
                // Parse UI
                DalllasWahProcessor.parse_ui(this.json_object.ui, this, DalllasWahProcessor.parse_item2);
    ...
    try {
        registerProcessor('DalllasWah', DalllasWahProcessor);
    } catch (error) {
        console.warn(error);
    }
`;
...
if (typeof module === "undefined") {
    window.DalllasWah = DalllasWah;
    window.FaustDalllasWah = DalllasWah;
} else {
    module.exports = { DalllasWah };
}
