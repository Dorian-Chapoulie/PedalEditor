...
with {
  Notches = 4;
  freq  = hslider("Speed [style: knob][unit:Hz][OWL:PARAMETER_A]", 0.5, 0.001, 20, 0.01);
  
  depth  = hslider("Depth [style: knob][OWL:PARAMETER_B]", 1, 0, 1, 0.01);
  fb     = hslider("Feedback Gain [style: knob][OWL:PARAMETER_C]", 0, 0, 1, 0.01);
  width  = hslider("Notch Width [style: knob] [unit:Hz][OWL:PARAMETER_D]", 1000, 10, 5000, 1);
  vibr   = checkbox("VibratoMode [enum:direct | vibrato]");
  frqmin = hslider("MinNotch1Freq [style: knob] [unit:Hz] ", 100, 20, 5000, 1);
  frqmax = hslider("MaxNotch1Freq [style: knob] [unit:Hz] ", 800, 20, 10000, 1) : max(frqmin);
  fratio = hslider("NotchFreq [style: knob]" , 1.5, 1.1, 4, 0.01);
  mdepth = select2(vibr,depth,2); 
  
  invert   = checkbox("invert[enum:linear|invert]");
  level  = hslider("level [style: knob] [unit:dB]", 0, -60, 10, 0.1) : db2linear;
  
};
PhaserRix = phaser_stereogx;
process= ba.bypass_fade(ma.SR/10, checkbox("bypass"),  PhaserRix);